...
void Driver::setHeadingAlignment(float headingAlignment)
{
    // Convert from radians to hundredths of a degree.
    int headingAlignmentInt = static_cast<int>(headingAlignment / M_PI * 18000);
    sendStandardCommand(std::string("EA"), headingAlignmentInt, 5, true);
}

void Driver::set_e_headingBias(float e_headingBias)
{
    // Convert from radians to hundredths of a degree.
    int headingBiasInt = static_cast<int>(e_headingBias / M_PI * 18000);
    sendStandardCommand(std::string("#EV"), headingBiasInt, 5, true, true);
}
...
void Driver::sendStandardCommand(const std::string& characters, int value, int numDigits, bool sign, bool expert)
{
    ...
    // Here numDigits is being used as the number of chars in the char array that is send.
    if (sign)
        ++numDigits;
    if (expert)
        ++numDigits;
